...
;;;
COMMENT ON TABLE jmrdemo.synth_metrics.order_reconciliation IS 'Reconciles REAL PizzaTel web orders (from the live OpenTelemetry storefront) to their rows in the synth pipeline. Grain: web_order_id. web_order_id = the storefront order UUID (app.order.id); guest_order_id = the bridged synth key that lands in synth_silver.guest_order. reconciled=TRUE means the web order has flowed through to silver (amount_diff should be ~0). web_amount/silver_total_amount let you audit value fidelity. Use this view to answer "did web order <UUID> make it into the data?", to count/verify how many real orders reconciled, or to distinguish real (source=otel) from synthetic orders. Only present when the OTel source is configured.'
;;;
