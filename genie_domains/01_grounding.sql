...
;;;
COMMENT ON TABLE jmrdemo.synth_metrics.order_reconciliation IS 'RECONCILIATION AUDIT view: does a REAL PizzaTel web order (from the live OpenTelemetry storefront) exist in the synth pipeline? Grain: web_order_id. web_order_id = the storefront order UUID (app.order.id); guest_order_id = the bridged synth key that lands in synth_silver.guest_order; reconciled=TRUE means the web order flowed through to silver (amount_diff ~0). Use ONLY for audit questions: "did web order <UUID> reach the data / reconcile?", "how many real orders reconciled?", real-vs-synthetic counts. For order DETAILS / ITEMS / STORE / CUSTOMER of a web order, do NOT use this view — use the live views synth_metrics.web_order_live + web_order_item_live instead (they are populated within seconds, before the order reaches silver). Only present when the OTel source is configured.'
;;;
COMMENT ON VIEW jmrdemo.synth_metrics.web_order_live IS 'LIVE, PIPELINE-INDEPENDENT header for REAL PizzaTel web orders, straight from OpenTelemetry — a web order appears here within seconds of being placed, BEFORE it reaches synth_silver via the hourly pipeline. Grain: web_order_id (the storefront app.order.id UUID). web_store_id / web_store_city / web_store_state / web_store_zip = the REAL storefront the guest ordered from (NOT a synth unit). web_amount = the true order total; web_item_count / web_total_quantity = line/unit counts. channel = fulfillment channel. order_stage = live fulfillment status (Prep, Bake, QualityCheck, OutForDelivery, Delivered). member_id = injected app.order.member_id (synth customer key; NULL = anonymous); customer_matched/customer_tier/customer_total_orders/customer_lifetime_spend from synth_features.customer_features. Join to web_order_item_live on web_order_id for line items. Use for "details / which customer / which store / status of web order <UUID>". Only present when the OTel source is configured.'
;;;
COMMENT ON VIEW jmrdemo.synth_metrics.web_order_item_live IS 'LIVE, PIPELINE-INDEPENDENT line items for REAL PizzaTel web orders, parsed straight from the OTel order span (order.skus). Grain: one row per web_order_id x menu_item. menu_item_id/item_name/category from synth_ref.menu_item; quantity = units ordered; unit_price = catalog base_price; line_amount = base_price * quantity. NOTE: SUM(line_amount) is a catalog-price estimate and need NOT equal the true order total web_order_live.web_amount (discounts/tax/promos live only on the header). Join to web_order_live on web_order_id for store/customer/amount/status. Available within seconds of the order, before it reaches silver. Only present when the OTel source is configured.'
;;;
