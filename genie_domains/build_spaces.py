...
"orders_sos": {
  ...
  "questions": sq([
     "Which stores have the most SOS breaches in the last 7 days?",
     "What is the SOS breach rate by channel across all stores this month?",
     "What was total revenue and average order value by channel last 30 days?",
     "Show daily revenue trend for store 36 over the last 30 days",
     "Which channel has the worst SOS compliance?",
     "What are the top menu items by revenue at store 105 this month?",
     "How many delivery orders were late this week, by store?",
     "What is the order cancellation rate by store this month?",
     "Compare store 36 and store 217 on revenue and SOS breach rate",
     "What are the busiest hours of the day for orders?",
     "Which franchisees have the highest revenue this month?",
     "Show SOS compliance percentage trend by week for the carryout channel"]),
  "instructions": ti([
     GLOSSARY,
...
"loyalty": {
  ...
  "questions": sq([
     "Show loyalty points earned vs redeemed by store this month",
     "What is the redemption rate by store over the last 30 days?",
     "How many active loyalty members are there by tier?",
     "Which tier redeems the most points?",
     "Compare average order value for members vs non-members",
     ...
     "Show the trend of points earned by week for gold-tier members",
     "What share of orders are placed by loyalty members?",
     "How many active digital accounts are there?",
     "Which franchisees have the highest loyalty engagement this month?",
     "What is the net points balance (earned minus redeemed) by store?"]),
  "instructions": ti([
     GLOSSARY,
...
"inventory": {
  ...
  "questions": sq([
     "Which stores have the highest waste cost this month?",
     "Show waste cost by category over the last 30 days",
     "What is the total waste cost and quantity this week?",
     "Which SKUs are currently below par at store 36?",
     "What is the receiving quality failure rate by store?",
     "How many receiving orders failed the temperature check this month?",
     "Show the waste cost trend by week for store 105",
     ...
     "How many replenishment orders are still open by store?",
...
"workforce": {
  ...
  "questions": sq([
     ...
     "Which employees worked overtime (over 40 hours) this week?",
     "How many unique employees worked at store 36 this month?",
     "What is the average hours per shift by store?",
     "Show the labor hours trend by week for store 105",
     "How many shifts were scheduled vs completed this week?",
     "Which stores are least labor-productive (lowest sales per labor hour)?",
     "What is total headcount by franchisee this month?",
     "Compare labor hours to revenue by store this week",
     "What is the average shift length by shift label?",
     "Which stores have the most overtime hours?"]),
  "instructions": ti([
     GLOSSARY,
...
def main():
    ...
    for key, d in DOMAINS.items():
        ...
        if d["title"] in have:
            sid = have[d["title"]]
            p = cli(["genie","update-space", sid, blob, "--title", d["title"], "--description", d["description"]])
            action = "UPDATED"
        else:
