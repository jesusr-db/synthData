...
│  │  4 UC Metric Views   │   │  ├── UC column tags  (pii/financial/sc)    │  │
│  │  (WITH METRICS YAML) │   │  ├── Column masks  (mask_email/phone)      │  │
│  └─────────────────────┘   │  ├── Row filters  (filter_by_franchisee)   │  │
│                             │  ├── UC Volume  (ref.assets)               │  │
│  ┌─────────────────────┐   │  └── Lakehouse Monitors (staging tables)   │  │
│  │  Genie Space         │   └────────────────────────────────────────────┘  │
│  │  10 seed questions   │                                                    │
│  │  all silver+metrics  │                                                    │
│  └─────────────────────┘                                                    │
...
| `QSR Destroy [dev]` | Job (on-demand) | Tears down all non-DAB objects: UC functions, volume, monitors, metric views, ref schema, metrics schema | Not deployed |
