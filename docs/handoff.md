...
| Catalog-level ABAC policies cause DLT to fail with `ABAC_POLICIES_NOT_SUPPORTED` on every update | ABAC catalog policies fail on ALL DLT updates (not just full_refresh) when the policy matches a DLT-owned table. | Removed `class.email_address`/`class.phone_number` tags from `silver.guest_profile` so ABAC only applies to `staging.guest_events` (not DLT-owned). `silver.guest_profile` uses per-table `SET MASK` instead (Step 5b in `apply_governance`). |
